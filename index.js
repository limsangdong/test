...
const sql_create_product = `CREATE TABLE IF NOT EXISTS Product (
  idx INTEGER PRIMARY KEY AUTOINCREMENT,
...
app.get("/books", (req, res) => {
  ...
  db.all(sql, [], (err, rows) => {
    ...
    for (var i = 0; i < rows.length; i++) {
      sumAmount += rows[i].amount
    }

    if (err) {
      return console.error(err.message);
    }
    console.log("123", sumAmount)
    res.render("book", { model: rows, moment: moment, axios: axios, sumAmount: sumAmount });
  });
});
...
app.post("/ExcelDownload/:dateofuse", (req, res) => {
  ...
  db.all(sql, date_of_use, (err, results) => {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      resultdata.push([results[i].date_of_use, results[i].return_date, results[i].user, results[i].where_to_use, results[i].usage_history,
      results[i].amount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "원",
      results[i].note, results[i].signature, sumAmount.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "원"]);
